...
def analyze(project_dir: Path, java_classes: Path, pypi_client=None, resolve_maven=False) -> dict:
    ...
    for path in sorted(project_dir.rglob("*")):
        ...
        if path.name == "requirements.txt":
            ...
            files.append(str(path.relative_to(project_dir)))
            command = ["java", "-cp", str(java_classes), "com.bomerang.JsonDependencyExporter", str(path)]
            if resolve_maven and path.name == "pom.xml":
                command.append("--resolve-maven")
            completed = subprocess.run(
                command,
                capture_output=True, text=True, check=True,
            )
            if resolve_maven and ("[X]" in completed.stderr or "[!] POM 다운로드 실패" in completed.stderr):
                raise RuntimeError(f"Maven remote resolution incomplete for {path}:\n{completed.stderr}")
...
def main():
    ...
    parser.add_argument("--resolve-maven", action="store_true", help="Fetch Maven POMs and include dependencies up to depth 3")
    args = parser.parse_args()
    print(json.dumps(analyze(args.project_dir, args.java_classes, resolve_maven=args.resolve_maven), ensure_ascii=False, indent=2))
